...
const RegistrationCard = ({ assetBaseUrl, gender, light, onOpen, onToggle, saving, signedIn, slot }: {
  ...
  const imageUri = gender === 'Female' ? slot.entry.femaleImageUri : slot.entry.imageUri;
...
export const NativePokedexDetailScreen = ({ allEntries, assetBaseUrl, entry, error = null, initialGender, isLoading = false, isSaving = false, onBack, onOpenEntry, onSetRegistrations, onToggleRegistration, pokemon, signedIn }: Props) => {
  ...
  const visibleGender = selectedGender && genders.includes(selectedGender)
    ...
    : initialGender && genders.includes(initialGender)
      ? initialGender
      : genders[0];
  const heroImageUri = visibleGender === 'Female' ? entry.femaleImageUri : entry.imageUri;
    ...
    <View style={[styles.root, light && styles.rootLight]} testID="native-pokedex-detail-screen">
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: 8 + insets.top, paddingBottom: 100 + insets.bottom }]} keyboardShouldPersistTaps="always" nestedScrollEnabled>
        ...
        {tab === 'registered' ? <View style={styles.tabPanel}>
          ...
          {groupedSlots.map((section) => <View key={section.key} style={styles.slotSection}><Text style={[styles.slotSectionTitle, light && styles.textLight]}>{section.label}</Text><View style={styles.variantGrid}>{section.slots.map((slot) => <RegistrationCard assetBaseUrl={assetBaseUrl} gender={visibleGender} key={slot.id} light={light} onOpen={() => onOpenEntry(slot.entry, visibleGender)} onToggle={() => onToggleRegistration(slot.registration, !slot.registered)} saving={isSaving} signedIn={signedIn} slot={slot} />)}</View></View>)}
        </View> : null}
