...
const entry = {
  ...
  imageUri: '/bulbasaur.png', typeIconUris: ['/images/types/grass.png'], maxKind: null,
...
describe('NativePokedexDetailScreen', () => {
  it('preserves registered, info, battle, and exact-combination workflows', () => {
    ...
    render(<SafeAreaProvider initialMetrics={{ frame: { x: 0, y: 0, width: 390, height: 844 }, insets: { top: 24, right: 0, bottom: 20, left: 0 } }}><NativePokedexDetailScreen allEntries={[entry]} assetBaseUrl="https://pokegonexus.com" entry={entry} onBack={jest.fn()} onManage={jest.fn()} onOpenEntry={jest.fn()} onToggleRegistration={onToggleRegistration} pokemon={pokemon} signedIn /></SafeAreaProvider>);

    expect(screen.getByText('Registration checklist')).toBeTruthy();
    fireEvent.press(screen.getByLabelText('Register Shiny Bulbasaur'));
    ...
    fireEvent.press(screen.getByText('Lucky'));
    expect(onToggleRegistration).toHaveBeenLastCalledWith(expect.objectContaining({ registrationId: '0001-shiny|lucky:true' }), true);
  });

  it('does not let a manual toggle remove a quality proved by a caught instance', () => {
    const onToggleRegistration = jest.fn();
    render(<SafeAreaProvider initialMetrics={{ frame: { x: 0, y: 0, width: 390, height: 844 }, insets: { top: 24, right: 0, bottom: 20, left: 0 } }}><NativePokedexDetailScreen allEntries={[{ ...entry, instanceRegistered: true, registered: true, registeredFacets: [{ lucky: true }] }]} assetBaseUrl="https://pokegonexus.com" entry={{ ...entry, instanceRegistered: true, registered: true, registeredFacets: [{ lucky: true }] }} onBack={jest.fn()} onManage={jest.fn()} onOpenEntry={jest.fn()} onToggleRegistration={onToggleRegistration} pokemon={pokemon} signedIn /></SafeAreaProvider>);
