...
const routePaths = configuredRoutePaths.length > 0 ? configuredRoutePaths : defaultRoutePaths;
...
test.describe('cross-browser route smoke', () => {
  test('loads core routes without browser runtime errors', async ({ page }, testInfo) => {
    ...
    try {
      for (const routePath of routePaths) {
        await test.step(`open ${routePath}`, async () => {
          ...
          const screenshotPath = path.join(screenshotsDir, `${slugifyRoute(routePath)}.png`);
