...
1. P2.1 Monorepo/package extraction prep
   ...
   - Status: In Progress (shared `users`, `search`, `auth`, `trades`, `location`, `events`, `pokemon` contracts extracted/wired; shared domain normalizers added; CI preflight guard added for package resolution; `UserOverview`, auth session DTOs, and `PartnerInfo` shapes centralized in shared contracts and consumed by web, 2026-02-21)
