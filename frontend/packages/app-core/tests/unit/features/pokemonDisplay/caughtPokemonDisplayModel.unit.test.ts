...
import { buildCaughtPokemonDisplayModel } from '@/features/pokemonDisplay/caughtPokemonDisplayModel';
...
describe('buildCaughtPokemonDisplayModel', () => {
  it('resolves display data in fusion, mega, crown order and builds preview Pokemon', () => {
    ...
    const result = buildCaughtPokemonDisplayModel({
      pokemon,
      variants: [
        {
          pokemon_id: 25,
    ...
    expect(result.resolvedFusionDisplay.source).toBe('base');
    expect(result.resolvedMegaDisplay.source).toBe('mega');
    expect(result.resolvedCrownDisplay.source).toBe('crown');
    expect(result.statsPokemon).toMatchObject({
      type1_name: 'Fairy',
      type2_name: 'Steel',
      sizes: crownSizes,
    });
    expect(result.movesPokemon.moves?.map((entry) => entry.name)).toEqual([
      'Crown Blast',
    ]);
    expect(result.movesPokemon.instanceData).toMatchObject({
      crown: true,
      is_fused: false,
      fast_move_id: 7,
      charged_move1_id: 8,
  ...
  it('uses fusion display and suppresses mega and crown display when fused', () => {
    ...
    expect(result.resolvedFusionMoves.source).toBe('fusion');
    expect(result.resolvedFusionDisplay.source).toBe('fusion');
    expect(result.resolvedMegaDisplay.source).toBe('base');
    expect(result.resolvedCrownDisplay.source).toBe('base');
    expect(result.statsPokemon).toMatchObject({
      type1_name: 'Ghost',
      type2_name: 'Psychic',
    });
