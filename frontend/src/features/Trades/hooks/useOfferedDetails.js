...
export function useOfferedDetails(trade, variants, relatedInstances) {
  ...
  useEffect(() => {
    ...
    const parsedReceiving = parsePokemonKey(trade.pokemon_instance_id_user_proposed);
    ...
    const existingReceivingDetails = relatedInstances
      ? relatedInstances[trade.pokemon_instance_id_user_proposed]
      : null;
