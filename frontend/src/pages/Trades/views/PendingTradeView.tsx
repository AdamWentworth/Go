...
interface PendingTradeViewProps {
...
const PendingTradeView: React.FC<PendingTradeViewProps> = ({
  trade,
  currentUserDetails,
  partnerDetails,
  loading,
  ...
  const [localTrade, setLocalTrade] = useState<TradeViewTrade>(trade);
  ...
  const userConfirmationField: CompletionField = isCurrentUserProposer
    ? 'user_proposed_completion_confirmed'
    : 'user_accepting_completion_confirmed';
  const partnerConfirmationField: CompletionField = isCurrentUserProposer
    ? 'user_accepting_completion_confirmed'
    : 'user_proposed_completion_confirmed';

  const hasUserConfirmed = Boolean(trade[userConfirmationField]);
  const hasPartnerConfirmed = Boolean(trade[partnerConfirmationField]);
  ...
  const leftHeading = 'Your Pokemon';
  const rightHeading = "Trade Partner's Pokemon";
  ...
  const handleRevealInfo = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to reveal partner info');
    } finally {
