...
export function getTypeEffectivenessMultiplier(
  ...
  defendingTypes: Array<string | null | undefined>,
): number {
  ...
  let multiplier = 1;
  const normalizedAttack = attackingType.toLowerCase();
  const attackMatchups = typeChart[normalizedAttack];

  defendingTypes.forEach((defendingType) => {
    if (!defendingType || !attackMatchups) return;
    const normalizedDefense = defendingType.toLowerCase();
    if (attackMatchups[normalizedDefense] !== undefined) {
      multiplier *= attackMatchups[normalizedDefense];
    }
  });
