...
def store_pvp_charged_moves():
    # Connect to the SQLite database
    connection = sqlite3.connect("data/pokego.db", timeout=30)
    cursor = connection.cursor()
    
    # Fetch the pvp_charged_moves data from the endpoint
    response = requests.get("https://pogoapi.net/api/v1/pvp_charged_moves.json")
    pvp_charged_moves = response.json()

    for move in pvp_charged_moves:
