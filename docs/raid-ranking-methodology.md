...
**Optimize lobby** performs a bounded coordinate search over legal six-Pokemon teams for the entire configured party. It compares complete shared-boss simulations rather than adding six independent DPS totals, so a candidate can win through better Mega/Primal uptime, fewer faints or relobbies, Party Power timing, or a faster clear. The search evaluates at most 160 coordinated lineups, keeps each Trainer's behavior settings, applies the winning teams back to the editor, and reports the measured improvement over the starting lobby. Monte Carlo configurations use the deterministic expected model while searching and then re-evaluate the winning lineup under the selected Monte Carlo settings before presenting it.
...
- Lobby optimization is a bounded coordinate search rather than an exhaustive proof over every possible six-Pokemon permutation. It can discover lineup interactions that individual rankings miss, but a larger offline optimizer may find a stronger party.
...
- Heterogeneous-party tests lock independent contribution, per-Trainer action delay and dodge outcomes, shared boss damage, active ally Mega boosts, legal six-member selection, unique roster slots, and the one-Mega-or-Primal team restriction. Optimizer tests prove that whole-lobby outcomes can select coordinated teams that independent defaults miss while enforcing the evaluation ceiling. Page tests exercise adding Trainers, running both party worker commands, applying optimized teams, rendering contributions, and carrying exact prediction provenance into observed-raid calibration.
...
2. Add an optional manual Party Power activation policy and measured action-delay profiles without treating raw ping as move delay.
3. Compare calibrated custom-party predictions against an additional independent full simulator before tightening matrix tolerances.
