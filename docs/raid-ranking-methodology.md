...
Boss counters use an event-driven, no-dodge battle simulation rather than the broad-list cycle formula. The engine models:
...
- attacker faints, one-second team swaps, six-Pokemon teams, and the selected relobby delay
...
- Boss counter trainer counts scale the one-Trainer simulated damage with a safety margin. They do not reproduce the additional boss energy generated by several Trainers attacking simultaneously.
- Boss action cadence and the 50% charged decision are represented by deterministic expected phases. Random streaks, dodging, network delay, mixed teams, staggered Mega uptime, Party Power meter timing, and relobby healing bugs can change a real raid result.
...
| A | PokeGo Nexus | Explainable general planning plus integrated boss lookup | Neutral all-type benchmark, legal form-specific move pools, independently scored tier-scaled type targets, exact damage floors, event-driven no-dodge boss counters, expected/favorable/hostile legal boss attacks, real two-move type effectiveness, Hidden Power handling, sortable component metrics, and adjustable modifiers | Broad lists remain closed-form; boss simulation uses deterministic expected phases and a bounded candidate screen rather than Monte Carlo over the full catalog |
...
PokeGo Nexus is a **strong A-tier broad ranking model with an event-driven boss-counter mode**. Its main advantage is auditability: the displayed eDPS, DPS, TDO, ER, CP, moveset, target selection, boss-pressure mode, and user modifiers all belong to one model instead of mixing a hidden score with editorial ordering. Overall uses a documented typeless benchmark so the historical boss roster cannot bias its movesets. By Type independently scores real high-tier raid targets instead of collapsing them into a same-typing aggregate.

Boss mode now closes the largest previous gap, but it is not yet a Monte Carlo simulator equivalent to Pokebattler. The clearest remaining path is:

1. Randomize boss action cadence and charged decisions across a reproducible Monte Carlo sample, then report percentile Time to Win, faints, and relobbies.
2. Add realistic-dodge modes and mixed six-Pokemon teams.
3. Model multi-Trainer boss-energy feedback and Party Power meter timing.
4. Expand or move the candidate screen to a worker so every legal attacker moveset can be simulated without blocking the UI.
5. Add a machine-readable comparison against at least one independent full simulator and publish model/catalog versions with every ranking.
