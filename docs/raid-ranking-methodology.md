...
Personalized rankings feed both generated teams and the heterogeneous custom-party simulator. Trainers can also opt into a local battle-calibration log from Boss mode. Each observation stores the boss, model and catalog versions, Trainer count, predicted and observed clear time, faints, relobbies, dodge attempts, successful dodges, and optional measured latency. When a custom party has just been simulated and the logged Trainer count matches it, calibration records that exact party prediction. The records stay in that browser and are scoped to the signed-in user; observations from an older simulation-model version are not applied to the current model.
...
- Heterogeneous-party tests lock independent contribution, per-Trainer action delay and dodge outcomes, shared boss damage, active ally Mega boosts, legal six-member selection, unique roster slots, and the one-Mega-or-Primal team restriction. Page tests exercise adding Trainers, running the party worker fallback, rendering contributions, and carrying the custom party size into observed-raid calibration.
...
Boss mode now includes reproducible Monte Carlo outcomes, legal mixed teams, heterogeneous Trainer parties, independent dodges and relobbies, active ally Mega uptime, shared boss-energy feedback, timed Party Power, a six-boss independent-reference matrix, and opt-in observed dodge calibration. Public cohorts are calibration rather than an assertion that unlike scalar metrics should match. It is not yet equivalent to a large dedicated Pokebattler simulation. The clearest remaining path is:
