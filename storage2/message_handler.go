...
// parseOptionalInt tries to convert a value to int, defaults to 0 if nil/invalid.
...
func parseOptionalTime(value string) *time.Time {
    ...
    if err != nil {
        logrus.Warnf("Failed to parse time %s: %v", value, err)
        return nil
    }
    return &t
}
...
func parseAndUpsertTrades(data map[string]interface{}) error {
    ...
    // Instead of defaulting to "N/A", we now store NULL if empty (per your request).
    ...
    for _, t := range tradeUpdates {
        ...
        tradeCancelledBy := fmt.Sprintf("%v", tradeData["trade_cancelled_by"])
        ...
        defaultsTrade := Trade{
            ...
            // Now TraceID is *string in your model, so this will store nil if empty
            TraceID: traceID,
        }
