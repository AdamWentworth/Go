...
callback_url="${2:-https://pokegonexus.com/api/auth/discord/callback}"
...
printf 'Configuring Discord OAuth in %s\n' "$env_file"
read -r -p 'Discord OAuth client ID: ' client_id
read -r -s -p 'Discord OAuth client secret (input hidden): ' client_secret
printf '\n'

if [[ ! "$client_id" =~ ^[0-9]+$ ]]; then
  printf 'The client ID must contain only digits.\n' >&2
  exit 1
fi
...
printf 'Discord OAuth settings installed.\n'
