...
COUNT(DISTINCT CASE WHEN is_caught = 1 THEN user_id END) AS caught_user_count`).
		...
		if err := tx.
			Table((PokemonInstance{}).TableName()).
			Select(`
COUNT(DISTINCT CASE WHEN is_caught = 1 AND disabled = 0 THEN user_id END) AS collector_user_count,
